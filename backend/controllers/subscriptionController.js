...
export const buyAirtime = asyncHandler(async (req, res) => {
  const { user, amount, phone, network, pin } = req.body;
  if (!user || !amount || !phone || !network || !pin)
  ...
  if (checkUser.pin !== JSON.parse(pin)) {
...
export const buyData = asyncHandler(async (req, res) => {
  const { user, plan, amount, phone, network, pin } = req.body;
  if (!user || !amount || !phone || !plan || !network || !pin)
